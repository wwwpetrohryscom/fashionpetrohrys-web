...
export async function generateMetadata({
  ...
  return buildMetadata({
    title: messages.pageMeta.perfumesTopOk.title,
    description: messages.pageMeta.perfumesTopOk.description,
    path: PATH,
    locale,
  });
}

export default async function PerfumesTopOkPage({ params }: LocalePageProps) {
  const locale = await resolveLocale(params);
  const messages = getMessages(locale);
      ...
      >
        <ol className="flex flex-wrap gap-1">
          ...
          <li className="text-neutral-700">
            {messages.perfumes.tiers.topOkShort}
          </li>
        </ol>
      </nav>
