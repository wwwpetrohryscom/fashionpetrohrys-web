...
export async function generateMetadata({
  ...
  return buildMetadata({
    title: messages.pageMeta.system.title,
    description: messages.pageMeta.system.description,
    path: "/system",
    locale,
  });
}
...
export default async function SystemPage({ params }: LocalePageProps) {
  const locale = await resolveLocale(params);
  const articles = localizedArticlesByCategory(locale, "system");
  ...
  return (
    <div className="space-y-12">
      ...
      <CtaBlock
        ...
        ctaHref={localizePath(locale, "/outfits")}
      />
    </div>
  );
}
